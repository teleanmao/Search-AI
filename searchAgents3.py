...
def foodHeuristic(state, problem):
    ...
    if capsules:
        ...
        min_food=((0,0),(0,0),99999)
        min_capsules=((0,0),(0,0),99999)
        food_capsules=((0,0),(0,0),99999)
        near=0
        ...
        for food in food_available:
            if (pos_food[1], food) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(pos_food[1], food)]
            else:
                dis=manhattanDistance(pos_food[1],food)
                problem.heuristicInfo[(pos_food[1],food)] = dis
            if(min_food[2] > dis):
                min_food = (pos_food[1],food,dis)



        for food in food_available:
            if (pos_capsule[1], food) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(pos_capsule[1], food)]
            else:
                dis=manhattanDistance(pos_capsule[1],food)
                problem.heuristicInfo[(pos_capsule[1],food)] = dis
            if(food_capsules[2] > dis):
                food_capsules = (pos_capsule[1],food,dis)


        if min_food[2] <= food_capsules[2]:
            near =min_food[2]
        else:
            near= food_capsules[2]
        ...
        for food in food_available:
            if (position, food) in problem.heuristicInfo:
                ...
                problem.heuristicInfo[(position,food)] = dis
            if(max_food[2] < dis):
                max_food = (position,food,dis)

        furtherfood= max_food[1]

        for capsule in capsules:
            if (furtherfood, capsule) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(furtherfood, capsule)]
            else:
                
                dis = manhattanDistance(furtherfood,capsule)
                problem.heuristicInfo[(furtherfood, capsule)] = dis
            if(min_capsules[2] > dis):
                min_capsules = (furtherfood,capsule,dis)


      

        hvalue = min_capsules[2] + near
