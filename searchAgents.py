...
class DeceptiveSearchAgentpi2(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        astarpi2 = getattr(search, 'aStarSearchpi2')
        # the Real goal
        ...
        # optc(s,gr)
        state_sgr, path_sgr = astarpi2(problem, s, gr, heuristic=nullHeuristic)
        optc_sgr = len(path_sgr)
        # optc(s,gi)
        state_sgi, path_sgi  = astarpi2(problem, s, gi, heuristic=nullHeuristic)
        optc_sgi = len(path_sgi)
        # optc(gr,gi)
        state_grgi,path_grgi = astarpi2(problem, gr, gi, heuristic=nullHeuristic)
        ...
        state_st, path_st = astarpi2(problem, s, t, heuristic=nullHeuristic)
        #get optimal path from t to gr
        state_tgr, path_tgr = astarpi2(problem, t, gr, heuristic=nullHeuristic)
...
class DeceptiveSearchAgentpi3(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        astarpi2 = getattr(search, 'aStarSearchpi2')
        astarpi3 = getattr(search, 'aStarSearchpi3')
        ...
        for i, state in enumerate(state_grgi):
            if (i>=beta):
                ...
                break 
        # get deceptive path 
        path_st = astarpi3(problem, s, t, gr, gi, alpha, heuristic=manhattanHeuristics)
        # get best path 
        state_tgr, path_tgr = astarpi2(problem, t, gr, heuristic=nullHeuristic)
...
def adjustdistance(capsules, x1, y1, x2, y2, d):
    
    for i in range(abs(x1 - x2)):
        for capsule in capsules:
            if x1 <= x2:
                if capsule[0] == x1 + i  and (capsule[1] == y1 or capsule[1] == y2):
                    d = d- 1
                    break
            else:
                if capsule[0] == x1 - i and (capsule[1] == y1 or capsule[1] == y2):
    ...
    for i in range(abs(y1 - y2)):
        for capsule in capsules:
            if y1 <= y2:
                if capsule[1] == y1 + i  and (capsule[0] == x1 or capsule[0] == x2):
                    d = d- 1
                    break
            else:
                if capsule[1] == y1 - i  and (capsule[0] == x1 or capsule[0] == x2):
...
def adjusthvalue(capsulelist, x1, y1, x2, y2, hvalue):
...
def foodHeuristic(state, problem):
    ...
    # Find maxmium distance between two foods.
    for food1 in food_available:
        for food2 in food_available:
            if (food1 == food2):
                pass
            else:
                distance = manhattanDistance(food1, food2)
                if (max_distance[2] < distance):
                    max_distance = (food1, food2, distance)
    ...
    if (max_distance[0] == (0, 0) and max_distance[1] == (0, 0)):
        ...
        x2, y2 = max_distance[0]
        
        d1 = manhattanDistance(position,max_distance[0])
        d2 = manhattanDistance(position,max_distance[1])
        
        d1 = adjustdistance(capsules, x1, y1, x2, y2, d1)
        

        #adjusting d2
        
        x2, y2 = max_distance[1]
        
    
        d2= adjustdistance(capsules, x1, y1, x2, y2, d2)
        
        start = (0,0)
        end = (0,0)
        if d1 <= d2:
            start = max_distance[0]
            end = max_distance[1]
        else:
            start = max_distance[1]
            end = max_distance[0]
        
        hvalue = manhattanDistance(position,start) + manhattanDistance(start,end)
        ...
        hvalue= adjusthvalue(capsulelist, x1, y1, x2, y2, hvalue)
