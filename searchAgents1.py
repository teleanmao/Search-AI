...
def manhattanHeuristics(position1, position2):
    #The Manhattan distance heuristic between two nodes.
    x1, y1 = position1
    x2, y2 = position2
...
class DeceptiveSearchAgentpi2(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        astar = getattr(search, 'aStarSearchpi2')
        # the Real goal
        self.goal = state.getFood().asList()[0]
        gr = self.goal
        # the possible goals
        self.possibleGoal = state.getCapsules()[0]
        gi = self.possibleGoal
        ...
        # optc(s,gr)
        path_sgr = astar(problem, s, gr, heuristic=nullHeuristic)[1]
        optc_sgr = len(path_sgr)
        # optc(s,gi)
        path_sgi  = astar(problem, s, gi, heuristic=nullHeuristic)[1]
        optc_sgi = len(path_sgi)
        # optc(gr,gi)
        state_grgi,path_grgi = astar(problem, gr, gi, heuristic=nullHeuristic)
        optc_grgi = len(path_grgi)
        # beta
        beta = (optc_grgi+optc_sgr-optc_sgi)/2
        
        for i, state in enumerate(state_grgi):
            if (i>=beta):
                t = state
                break 

        #get optimal path from s to t
        path_st = astar(problem, s, t, heuristic=nullHeuristic)[1]
        #get optimal path from t to gr
        path_tgr = astar(problem, t, gr, heuristic=nullHeuristic)[1]

        
        self.actions = path_st + path_tgr
...
class DeceptiveSearchAgentpi3(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        # use A* search to compute optimal path
        astar = getattr(search, 'aStarSearchpi2')
        astarDecep = getattr(search, 'aStarSearchpi3')
        ...
        for i, state in enumerate(state_grgi):
            if (i>=beta):
                ...
                break 
        # get deceptive path 
        path_st = astarDecep(problem, s, t, gr, gi, alpha, heuristic=manhattanHeuristics)
        # get best path 
        path_tgr = astar(problem, t, gr, heuristic=nullHeuristic)[1]

        #combine path from s to t then to gr
        self.actions = path_st + path_tgr
        return self.actions
...
def foodHeuristic(state, problem):
    ...
    if (not food_available) and (not capsules):
        return 0
    
    if capsules:
        pos_capsule=((0,0),(0,0),99999)
        pos_food=((0,0),(0,0),99999)
        min_distance=((0,0),(0,0),99999)
        max_capsules=((0,0),(0,0),0)

        for food in food_available:
            if (position, food) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(position, food)]
            else:
                dis=manhattanDistance(position,food)
                problem.heuristicInfo[(position,food)] = dis
            if(pos_food[2] > dis):
                pos_food = (position,food,dis)

        for capsule in capsules:
            if (position, capsule) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(position, capsule)]
            else:
                dis=manhattanDistance(position,capsule)
                problem.heuristicInfo[(position,capsule)] = dis
            if(pos_capsule[2] > dis):
                pos_capsule = (position,capsule,dis)


        if (pos_food[2]< pos_capsule[2]):
            nearpos = pos_food
            food_available.remove(pos_food[1])
        else: 
            nearpos = pos_capsule
        ...
        for capsule in capsules:
            if (nearpos[1], capsule) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(nearpos[1], capsule)]
            else:
                
                dis = manhattanDistance(nearpos[1],capsule)
                problem.heuristicInfo[(nearpos[1], capsule)] = dis
            if(max_capsules[2] < dis):
                max_capsules = (nearpos[1],capsule,dis)


        nearfood = max_capsules[1]
        
        for food in food_available:
            
            if (nearfood, food) in problem.heuristicInfo:
                dis= problem.heuristicInfo[(nearfood, food)]
            else:    
                
                dis = manhattanDistance(nearfood,food)
                problem.heuristicInfo[(nearfood, food)] = dis
            if(min_distance[2] > dis):
                min_distance = (nearfood,food,dis)

        hvalue = min_distance[2] + nearpos[2]
...
'''
    ...
    # Base case 
    if (not foodlist) and (not capsules):
        return 0
   
    # If there are capsules, it will return the manhattan distance of the pacman and the furtherest capsules. 
    #if capsules:
        
        #for capsule in capsules:
        #    distances.append(get_distance(position, capsule))
            #x1,y1 = position
            #x2,y2 = capsule
            #xcoor.append(abs(x1-x2))
            #ycoor.append(abs(y1-y2))
    ...
    for food in foodlist:
        ...
        if (position, food) in problem.heuristicInfo:
            ...
            distance = eucledian(position, food)
    ...
    for capsule in capsules:
        capsuledistance= eucledian(position, capsule)
        capsuledistances.append(capsuledistance)

    if not capsuledistances:
        capsuledistances.append(0)
    if not distances:
        return 0 
    result = abs( max(distances)-max(capsuledistances))
